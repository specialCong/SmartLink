...
public class DeviceListFragment extends ListFragment {  // callback of requestPeers
    ...
    private class WiFiPeerListAdapter extends ArrayAdapter<WifiP2pDevice> {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (device != null) {
                ...
                LogTrace.d(TAG, "WiFiPeerListAdapter"," getView : " + device.deviceName);
            }
            return v;
        }
    }
    ...
    public void updateThisDevice(WifiP2pDevice device) { // callback of this device details changed bcast event.
    	...
    	if ( device != null) {
    		LogTrace.d(TAG, "updateThisDevice: ", device.deviceName + " = " + ConnectionService.getDeviceStatus(device.status));
	    	this.device = device;
    ...
    public void onPeersAvailable(List<WifiP2pDevice> peerList) {   // the callback to collect peer list after discover.
        ...
        if (peers.size() == 0) {
        	LogTrace.d(WiFiDirectActivity.TAG, "onPeersAvailable"," No devices found");
            return;
        }
    }
    ...
    public void clearPeers() {
    	getActivity().runOnUiThread(new Runnable() {
    		@Override public void run() {
    	        ...
    	        peers.clear();
    	        ((WiFiPeerListAdapter) getListAdapter()).notifyDataSetChanged();
    	        Toast.makeText(mApp,"p2p connection broken...please try again...", Toast.LENGTH_SHORT).show();
    		}
    	});
